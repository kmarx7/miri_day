...
export default function CategoryListScreen({ category, items, isSample, onBack }) {
  ...
  return (
    <div className="pt-5">
      ...
      <section aria-labelledby="active-items-heading">
        ...
        <ItemList items={activeItems} emptyMessage="진행 중인 항목이 없어요." />
      </section>
      ...
      {completedItems.length > 0 && (
        <section className="mt-8" aria-labelledby="completed-items-heading">
          ...
          <ItemList items={completedItems} emptyMessage="완료한 항목이 없어요." />
        </section>
      )}
    </div>
  )
